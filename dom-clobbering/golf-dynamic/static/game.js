...
function distanceToWall(ballMajor, ballMinor, normMajor, normMinor, wall) {
  ...
  if (
    wall[1][0] - ball.RADIUS - DELTA < minorLoc &&
    wall[1][1] + ball.RADIUS + DELTA > minorLoc
  ) {
    return dist;
  } else {
    return null;
...
function updateCanvas() {
  ...
  ctx.drawImage(
    holeSvg,
    (hole.x - hole.WIDTH / 2) * scaleX,
    (hole.y - hole.HEIGHT / 2) * scaleY,
    hole.WIDTH * scaleX,
    hole.HEIGHT * scaleY,
  );
  ctx.drawImage(
    ballSvg,
    (ball.x - ball.WIDTH / 2) * scaleX,
    (ball.y - ball.HEIGHT / 2) * scaleY,
    ball.WIDTH * scaleX,
    ball.HEIGHT * scaleY,
  );
...
async function startGame() {
  ...
  if (ballInHole()) {
    // Lucky user gets a cosmic bit flip??
    win();
  }
}
